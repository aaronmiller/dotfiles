...
nix-sync () {
    ...
    then
        ...
        quoted_modules=$(printf ',"%s"' "${modules[@]}") 
        extra_modules_json="[${quoted_modules:1}]" 
    fi

    # Pass the formatted JSON to sync.sh
    EXTRA_MODULES="$extra_modules_json" \
    $BASE_DOTFILES_DIR/sync.sh -d "$DOTFILES_DIR" "${passthru_args[@]}"
}
...
nix-apply() {
  ...
  EXTRA_MODULES="$extra_modules_json" \
  HOST=$(hostname) \
  darwin-rebuild switch --flake "${DOTFILES_DIR:-$HOME/dotfiles}#$(hostname)" --impure "${passthru_args[@]}"
}
